...
// 	Difficulty activeDifficulty = Settings::diffs[difficultyIndex];

// 	Maps maps(sheet, activeDifficulty);
// 	maps.generate();

// 	Details details(sheet, activeDifficulty.mineCount, activeDifficulty.dimentions, 1.f);
// 	ButtonHandler buttonHandler(sheet, details.getInfoPosition(), details.getInfoPadding(), 0.75f);
	
// 	ReplayManager replays(sheet, activeDifficulty, "sdmc:/mineplacer/");
// 	details.textPanel.loadLeaderboardText(`scores);
...
// 			if (!submittedTime)
// 			{
// 				replays.saveReplay(replays.recorder.getMoves(), maps.mineMap, timer.getTime(), activeDifficulty);
...
// 				buttonHandler.setVector();
// 				if (buttonHandler.helpText)
// 				{
// 					details.textPanel.currentScreen = helpScreen;
// 					details.textPanel.selection.x = difficultyIndex;
// 				}
// 				else if (!buttonHandler.helpText) details.textPanel.currentScreen = lbScreen;
...
// 			(replays.player.playing) ? activeDifficulty.mineCount - replays.player.getMinesPlaced() : activeDifficulty.mineCount - maps.minesPlaced,
